...
def main(content_dir_fp: str = 'content_data',
         ...
         embedder_out_dir: str = 'data/embedder',
         ):
    """
    ...
    #     mkdir(inter_out_dir)
    # if not os.path.isdir(embedder_out_dir):
    #     makedirs(embedder_out_dir)

    # Load content data
    loaded_content_data = content_preprocessing.load_contents_from_folder(content_dir_fp, subset=True)
    ...
    # Build embedder
    gensim_embedder = tokenizer_embedder.load_embedder() #gensim_embedder = tokenizer_embedder.build_embedder(corpus)
    # Vectorize textuals
    ...
    print('Content Data Saved')
    #tokenizer_embedder.save_embedder(gensim_embedder, f'{embedder_out_dir}/embedder_model')
    ...
    start_datapoints = 0
    end_datapoints = 0
    for file in tqdm(os.listdir(interaction_dir_fp), desc='Processing Interaction Files'):
    ...
    print(f'Saved embedder to folder: {embedder_out_dir}')
...
def combine_df_and_remove_infrequent_users(folder_fp, out_fp):
    ...
    print(
        f'Combined all dataframes into one and filtered out all with interactions_num < {globals.data_params["min_history_size"]}.')
...
def combine_df_and_remove_infrequent_users_from_list(data_list:list):
    df = pd.DataFrame()
    # combine
    for csv in tqdm(data_list):
        try:
            df = pd.concat([df, interaction_preprocessing.load_data_from_csv(csv)])
        except (UnicodeDecodeError, pd.errors.ParserError) as e:
            print(e)
            print(f'Tried to load_csv from {csv}. Failed.')
            raise e
...
if __name__ == '__main__':
    main(content_dir_fp='home/lemeiz/content_refine',
         content_out_fp='content_data.csv',
         interaction_dir_fp='interaction_data',
         inter_out_dir='preprocessed_interaction_data',
         embedder_out_dir='embedder',
         )
